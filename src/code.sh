...
# make output folders
mkdir -p ~/out/varscan_vcf/output ~/out/varscan_vcf_bed/output
...
# if not empty perform variant calling
	...
	$java -jar /usr/bin/VarScan.v2.4.3.jar mpileup2cns $opts > ~/out/varscan_vcf/output/${bam_file_prefix[i]}.varscan.vcf
